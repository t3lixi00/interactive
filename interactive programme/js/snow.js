/**
 * jQuery snow effects.
 *
 * This is a heavily modified, jQuery-adapted, browser-agnostic version of
 * "Snow Effect Script" by Altan d.o.o. (http://www.altan.hr/snow/index.html).
 *
 * Dustin Oprea (2011)
 */

function __ShowSnow(settings)
{

    var snowsrc = settings.SnowImage;
    var nu = settings.Quantity;

    var dx, x, y;    // coordinate and position variables
    var am, sx, sy;  // amplitude and step variables
    var i;

    var doc_width = $(window).width() - 12;
    var doc_height = $(window).height();

    dx = [];
    x = [];
    y = [];
    am = [];
    sx = [];
    sy = [];
    flakes = [];
    for (i = 0; i < nu; ++i)
    {
        dx[i] = 0;                        // set coordinate variables
        x[i] = Math.random()*(doc_width-50);  // set position variables
        y[i] = Math.random()*doc_height;
        am[i] = Math.random()*20;         // set amplitude variables
        sx[i] = 0.02 + Math.random()/10; // set step variables
        sy[i] = 0.7 + Math.random();     // set step variables

        var flake = $("<div />");

        var id = ("dot" + i);
        flake.attr("id", id);
        flake.css({
                    position: "absolute",
                    zIndex: i,
                    top: "15px",
                    left: "15px"
                });

        flake.append("<img src='" + snowsrc + "'>");
        flake.appendTo("body");

        flakes[i] = $("#" + id);
    }

    var animateSnow;
    animateSnow = function()
    {
        for (i = 0; i < nu; ++ i)
        {
            // iterate for every dot
            y[i] += sy[i];
            if (y[i] > doc_height - 50)
            {
                x[i] = Math.random() * (doc_width - am[i] - 30);
                y[i] = 0;
                sx[i] = 0.02 + Math.random() / 10;
                sy[i] = 0.7 + Math.random();
            }

            dx[i] += sx[i];
            flakes[i].css("top", y[i] + "px");
            flakes[i].css("left", (x[i] + am[i] * Math.sin(dx[i])) + "px");
        }

        snowtimer = setTimeout(animateSnow, 10);
    };

	function hidesnow()
    {
		if(window.snowtimer)
            clearTimeout(snowtimer)

        for (i = 0; i < nu; i++)
            flakes[i].hide();
	}

    animateSnow();
	if (settings.HideSnowTime > 0)
    	setTimeout(hidesnow, settings.HideSnowTime * 1000)
}

(function($) {
    $.fn.snow = function(options) {

    var settings = $.extend({
            SnowImage:      undefined,
            Quantity:       20,
            HideSnowTime:   0
        }, options);

    __ShowSnow(settings);

    return this;
  }

})(jQuery);
